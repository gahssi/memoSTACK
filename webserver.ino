/*
 ...
 The Serial monitor wil display the IP address of your WiFi module, which you can open in a web browser
 at the following URL:
 	http://yourAddress/H (turns the LED on)
 	http://yourAddress/L (turns it off)

 This code will work normally for a network using WPA/WPA2 encryption. 
 For WEP or WPA networks, change the Wifi.begin() call accordingly.
...
// PLEASE enter your sensitive network data in the Secret tab/network_secrets.h
				...
				if (c == '\n') {                    // if the byte is a newline character
					...
					if (currentLine.length() == 0) {
						...
						if (alarmSet == 0) {
							...
							// possible reminder times outlined in two column table, the first column for reminders on the current day
							// and the second column for reminders on the next day
						...
						if (alarmSet == 1) {
							...
							client.print(":");
							if (alarmMinute < 10)    // add a leading '0' for the first 10 seconds of each alarmMinute
								client.print("0");
...
void wifiSetup(){
	if (WiFi.status() == WL_NO_MODULE) {
		...
		// do not proceed if Wifi module not found
		while (true);
	}
...
void printTime(){
	...
	Serial.print(':');
	if (localMin < 10)
		Serial.print('0'); // add leading '0' for the first 10 alarmMinutes of each alarmHour
	
	Serial.print(localMin); // print the alarmMinute (3600 equals secs per alarmMinute)
	Serial.print(':');
	if (localSec < 10)
		Serial.print('0'); // add leading '0' for the first 10 seconds of each alarmMinute
...
void time(){
	...
	if (Udp.parsePacket()) {
		...
		// the timestamp starts at byte 40 of the received packet and is two words long
		// extract the two words first:
